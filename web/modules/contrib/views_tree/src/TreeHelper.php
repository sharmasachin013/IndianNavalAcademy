...
class TreeHelper {
   ...
   */
  protected ViewsResultTreeValues $treeValues;
   ...
   */
  public function buildRenderTree(ViewExecutable $view, array $rows): TreeItem {
    $result = $view->result;
   ...
   */
  public function addDataAttributes(TreeItem $tree, $nesting = 0): void {
    $node = $tree->getNode();
   ...
   *
   * @param array $result
   *   The views result with views_tree_main and views_tree_parent set.
   *
   * @return \Drupal\views_tree\TreeItem
   *   A tree representation.
   */
  public function getTreeFromResult(array $result): TreeItem {
   ...
   */
  protected function getTreeFromGroups(array $groups, string $current_group = '0'): TreeItem {
   ...
   */
  protected function groupResultByParent(array $result): array {
    $return = [];

    foreach ($result as $row) {
      if (property_exists($row, 'views_tree_parent') && isset($row->views_tree_main)) {
        $return[$row->views_tree_parent][$row->views_tree_main] = $row;
      }
    }
    return $return;
  }
   ...
   *   The new tree item.
   */
  public function applyFunctionToTree(TreeItem $tree, callable $callable): TreeItem {
    if (($tree->getNode())) {
